...
CREATE TABLE `asset` (
  ...
  KEY `ASSET_SUB_TYPE_ID_FK_idx1` (`ASSET_SUB_TYPE_ID`),
  ...
  CONSTRAINT `PO_ID_FK1` FOREIGN KEY (`PO_ID`) REFERENCES `po` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `request_history` (
  ...
  CONSTRAINT `REQUEST_ID_FK2` FOREIGN KEY (`REQUEST_ID`) REFERENCES `request` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `asset_issue` (
  ...
  `UPDATED_TIME` datetime DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `REQUEST_ID_FK_idx1` (`REQUEST_ID`),
  KEY `ISSUER_ID_FK_idx` (`ISSUED_BY`),
  KEY `RETURNED_BY_FK_idx` (`RETURNED_BY`),
  CONSTRAINT `ASSET_ID_FK` FOREIGN KEY (`ASSET_ID`) REFERENCES `asset` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ISSUED_BY_FK` FOREIGN KEY (`ISSUED_TO`) REFERENCES `users` (`USERNAME`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `ISSUED_BY_FK1` FOREIGN KEY (`ISSUED_BY`) REFERENCES `users` (`USERNAME`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `REQUEST_ID_FK` FOREIGN KEY (`REQUEST_ID`) REFERENCES `request` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE,
